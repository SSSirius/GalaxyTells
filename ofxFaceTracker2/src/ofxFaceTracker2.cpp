...
vector<float> ofxFaceTracker2::eyedist(){
    ofVec2f dis;
    vector<float> dis_ary;
    for (auto instance : getInstances()){
        //eyedist middle 39,42
        dis =  instance.getLandmarks().getImagePoint(39)-instance.getLandmarks().getImagePoint(42);
        dis_ary.push_back(dis.length());
    }
    return dis_ary;
}
vector<float> ofxFaceTracker2::eyesize(){
    ofVec2f dis;
    vector<float> dis_ary;
    for (auto instance : getInstances()){
        //eye 39,36
        dis =  instance.getLandmarks().getImagePoint(39)-instance.getLandmarks().getImagePoint(36);
        dis_ary.push_back(dis.length());
    }
    return dis_ary;
}
vector<float> ofxFaceTracker2::mouthdist(){
    ofVec2f dis;
    vector<float> dis_ary;
    for (auto instance : getInstances()){
        //mouth 48,54
        dis =  instance.getLandmarks().getImagePoint(48)-instance.getLandmarks().getImagePoint(54);
        dis_ary.push_back(dis.length());
    }
    return dis_ary;
}
vector<float> ofxFaceTracker2::mouthvertidist(){
    ofVec2f dis;
    vector<float> dis_ary;
    for (auto instance : getInstances()){
        //nose 2,314
        dis =  instance.getLandmarks().getImagePoint(51)-instance.getLandmarks().getImagePoint(57);
        dis_ary.push_back(dis.length());
    }
    return dis_ary;
}

vector<float> ofxFaceTracker2::nosedist(){
    ofVec2f dis;
    vector<float> dis_ary;
    for (auto instance : getInstances()){
        //nose 27,33
        dis =  instance.getLandmarks().getImagePoint(27)-instance.getLandmarks().getImagePoint(33);
        dis_ary.push_back(dis.length());
    }
    return dis_ary;
}
vector<float> ofxFaceTracker2::facedist(){
    ofVec2f dis;
    vector<float> dis_ary;
    for (auto instance : getInstances()){
        //nose 2,314
        dis =  instance.getLandmarks().getImagePoint(2)-instance.getLandmarks().getImagePoint(14);
        dis_ary.push_back(dis.length());
    }
    return dis_ary;
}
void ofxFaceTracker2::drawMesh(int x, int y) const{
    drawMesh(x, y, info.inputWidth, info.inputHeight);
}

void ofxFaceTracker2::drawMesh(int x, int y, int _w, int _h) const{
    if(failed) {
        return;
    }
...
//            ofDrawCircle(instance.getLandmarks().getImagePoint(48),3); ofDrawCircle(instance.getLandmarks().getImagePoint(54),3);
//
//            //mouth up down 51,57
//            ofDrawCircle(instance.getLandmarks().getImagePoint(51),3); ofDrawCircle(instance.getLandmarks().getImagePoint(57),3);
//
//            ofDrawCircle(instance.getLandmarks().getImagePoint(27),3); ofDrawCircle(instance.getLandmarks().getImagePoint(33),3);
            ofNoFill();
...
ofMatrix4x4 ofxFaceTracker2::getPoseMatrix(){
    ofMatrix4x4 mat;
    for(auto instance : getInstances()){
        mat = instance.getPoseMatrix();
        mat.scale(6,-6,1);
